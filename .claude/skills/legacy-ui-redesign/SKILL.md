...
- Which tokens are theme-aware and which aren't. In this project, `--gp-ink` and `--gp-text` have **no dark-mode override** — they're a trap left over from an early pass. Always use `--gp-fg` / `--gp-fg-muted` / `--gp-fg-faint` for text instead; these do adapt across `[data-theme]` and `prefers-color-scheme`. If the target design system has similar non-adaptive legacy tokens sitting next to theme-aware ones, identify them here before writing any CSS.
...
- Correct in both light and dark mode — not just visually plausible in whichever theme happens to be active while you're working
...
**0b. Second constraint — correct in both light and dark mode**

A redesign that only works in one theme is not done. This project's dark mode is not automatic — it depends on using the right tokens.

- Use `--gp-fg` / `--gp-fg-muted` / `--gp-fg-faint` for text. Never `--gp-ink` or `--gp-text` — those have no dark-mode value and will render as near-invisible or wrong-contrast text the moment a user switches themes.
- Inline SVGs frequently hardcode `stroke="#hex"` or `fill="#hex"` in the original markup. Replace with `currentColor` (inheriting a `--gp-*` text token) or an explicit theme-aware override — a leftover hardcoded stroke color is the single most common way a "redesigned" icon goes invisible in dark mode.
- Any new background, border, or shadow must resolve through a token that has a dark-mode value (`--gp-surface`, `--gp-border`, `--gp-shadow-card`, etc.), not a literal hex or an rgba() picked to look right in light mode only.
- Before calling any phase done: toggle to dark mode and re-look at every element that phase touched, not just the ones that seemed risky.

**Process**
- Audit first. List every live page in the nav section before changing markup.
- Work phase by phase. Stop after each phase for a visual check — in **both light and dark mode**, not just whichever theme is currently active. Toggle it via `data-theme` on `.gp-ds` (or `localStorage['gp-ds-theme']`, per `gp-theme-toggle.js`) and re-check every element you touched: text contrast, icon/SVG fills (`stroke`/`fill` attributes often hardcode a light-mode color), borders, and any hardcoded hex left over from the legacy screen. A phase is not done until both themes have been checked, not just built and glanced at once.
