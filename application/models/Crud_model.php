...
class Crud_model extends CI_Model
{
    ...
    {
        return $this->db->get_where('category', ['id' => $id]);
    }

    public function get_category_id($slug = "")
    {
        $category_details = $this->db->get_where('category', ['slug' => $slug])->row_array();
    ...
    {
        $this->db->where('id', $category_id);
        $this->db->update('category', ['sub_category_thumbnail' => null]);
    }

    public function get_sub_categories($parent_id = "")
    {
        return $this->db->get_where('category', ['parent' => $parent_id])->result_array();
    }
    ...
    public function enrol_history($course_id = "", $distinct_data = false)
    {
        if ($distinct_data) {
            ...
            if ($course_id > 0) {
                return $this->db->get_where('enrol', ['course_id' => $course_id]);
            } else {
                return $this->db->get('enrol');
            }
        }
    ...
    {
        return $this->db->get_where('enrol', ['user_id' => $user_id]);
    }
    ...
    {
        if ($user_id > 0) {
            return $this->db->get_where('payment', ['user_id' => $user_id]);
        } else {
    ...
    {
        return $this->db->get_where('payment', ['id' => $payment_id])->row_array();
    }
    ...
    public function get_payment_gateways($identifier = "")
    {
        if ($identifier) {
    ...
    public function get_lessons($type = "", $id = "")
    {
        ...
        if ($type == "course") {
            ...
            return $this->db->get_where('lesson', ['section_id' => $id]);
        } elseif ($type == "lesson") {
            return $this->db->get_where('lesson', ['id' => $id]);
        } else {
    ...
    public function get_my_courses_by_category_id($category_id)
    {
        ...
        $course_lists_by_enrol = $this->db->get_where('enrol', ['user_id' => $this->session->userdata('user_id')])->result_array();
        $course_ids            = [];
    ...
    {
        return $this->db->get_where('course', ['id' => $course_id]);
    }
    ...
    public function delete_course($course_id = "")
    {
        ...
        if ($course_type == 'general') {
            ...
            $lesson_checker = ['course_id' => $course_id];
    ...
    public function get_section($type_by, $id)
    {
        ...
        if ($type_by == 'course') {
            ...
            return $this->db->get_where('section', ['id' => $id]);
        }
    }
    ...
    public function edit_lesson($lesson_id)
    {
        ...
        $previous_data = $this->db->get_where('lesson', ['id' => $lesson_id])->row_array();
        ...
        if ($lesson_type == 'video') {
            ...
            if ($attachment_type == 'audio') {
                if (isset($_FILES['system_video_file']) && ! empty($_FILES['system_audio_file']['name'])) {
                    ...
                    $previews_audio_url = $this->db->get_where('lesson', ['id' => $lesson_id])->row('audio_url');
                ...
                if (isset($_FILES['system_video_file']) && ! empty($_FILES['system_video_file']['name'])) {
                    ...
                    $previews_video_url = $this->db->get_where('lesson', ['id' => $lesson_id])->row('video_url');
    ...
    public function delete_lesson($lesson_id)
    {
        ...
        $watch_history = $this->db->get_where('watch_histories', ['student_id' => $user_id, 'course_id' => $previous_data['course_id']]);
    ...
    {
        if ($this->db->get_where('rating', ['user_id' => $data['user_id'], 'ratable_id' => $data['ratable_id'], 'ratable_type' => $data['ratable_type']])->num_rows() == 0) {
            $this->db->insert('rating', $data);
            return 'added';
        } else {
            $checker = ['user_id' => $data['user_id'], 'ratable_id' => $data['ratable_id'], 'ratable_type' => $data['ratable_type']];
    ...
    {
        $reviews = $this->db->get_where('rating', ['ratable_type' => $ratable_type, 'user_id' => $this->session->userdata('user_id'), 'ratable_id' => $ratable_id]);
    ...
    public function get_ratings($ratable_type = "", $ratable_id = "", $is_sum = false)
    {
        if ($is_sum) {
            ...
            return $this->db->order_by('id', 'desc')->get_where('rating', ['ratable_type' => $ratable_type, 'ratable_id' => $ratable_id]);
        } else {
            return $this->db->order_by('id', 'desc')->get_where('rating', ['ratable_type' => $ratable_type, 'ratable_id' => $ratable_id]);
        }
    }
    ...
    {
        $number_of_user_rated = $this->db->get_where('rating', [
            'ratable_type' => $ratable_type,
            'ratable_id'   => $ratable_id,
        ])->num_rows();

        $number_of_user_rated_the_specific_rating = $this->db->get_where('rating', [
            'ratable_type' => $ratable_type,
            'ratable_id'   => $ratable_id,
            'rating'       => $rating,
        ])->num_rows();
    ...
    public function send_new_private_message()
    {
        ...
        $num1 = $this->db->get_where('message_thread', ['sender' => $sender, 'receiver' => $receiver])->num_rows();
        $num2 = $this->db->get_where('message_thread', ['sender' => $receiver, 'receiver' => $sender])->num_rows();
        if ($num1 == 0 && $num2 == 0) {
        ...
        if ($num1 > 0) {
            $message_thread_code = $this->db->get_where('message_thread', ['sender' => $sender, 'receiver' => $receiver])->row()->message_thread_code;
        }

        if ($num2 > 0) {
            $message_thread_code = $this->db->get_where('message_thread', ['sender' => $receiver, 'receiver' => $sender])->row()->message_thread_code;
        }
    ...
    public function send_reply_message($message_thread_code)
    {
        ...
        $message_thread = $this->db->get_where('message_thread', ['message_thread_code' => $message_thread_code])->row_array();
    ...
    public function mark_thread_messages_read($message_thread_code)
    {
        ...
        $this->db->update('message', ['read_status' => 1]);
    }
    ...
    // version 1.4
    public function filter_course($search_string = "", $selected_category_id = "", $selected_price = "", $selected_level = "", $selected_language = "", $selected_rating = "", $selected_sorting = "", $per_page = "", $uri_segment = "")
    {
    ...
    public function get_courses($category_id = "", $sub_category_id = "", $instructor_id = 0)
    {
        if ($category_id > 0 && $sub_category_id > 0 && $instructor_id > 0) {
            ...
            return $this->db->get_where('course', ['category_id' => $category_id, 'sub_category_id' => $sub_category_id]);
        } else {
            return $this->db->get('course');
        }
    }
    ...
    public function manage_mcq_choice_question($quiz_id, $question_id, $action)
    {
    ...
    public function manage_plain_text_question($quiz_id, $question_id, $action)
    {
        $data['title']             = htmlspecialchars_($this->input->post('title', false));
        $data['type']              = htmlspecialchars_($this->input->post('question_type'));
    ...
    public function manage_fill_in_the_blank_question($quiz_id, $question_id, $action)
    {
        if (empty($this->input->post('title'))) {
            echo get_phrase('question_title_can_not_be_empty');
            return;
    ...
    public function update_watch_history_manually($lesson_id = "", $course_id = "", $user_id = "")
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if (! in_array($lesson_id, $lesson_ids)) {
                ...
                $this->db->update('watch_histories', ['course_progress' => $course_progress, 'completed_lesson' => json_encode($lesson_ids), 'completed_date' => $completed_date, 'date_updated' => time()]);
                $is_completed = 1;
            } else {
    ...
    public function enrol_to_free_course_mobile($course_id = "", $user_id = "")
    {
        ...
        if ($this->db->get_where('course', ['id' => $course_id])->row('is_free_course') == 1):
            $data['gifted_by'] = 0;
            if ($this->db->get_where('enrol', ['course_id' => $course_id, 'user_id' => $user_id])->num_rows() > 0) {
                $data['course_id']  = $course_id;
                $data['user_id']    = $user_id;
    ...
    {
        return $this->db->get_where('enrol', ['course_id' => $course_id, 'user_id' => $user_id])->num_rows();
    }
    ...
    public function check_duplicate_payment_for_stripe($transaction_id = "", $stripe_session_id = "", $user_id = "")
    {
        ...
        $query = $this->db->get_where('payment', ['user_id' => $user_id, 'transaction_id' => $transaction_id, 'session_id' => $stripe_session_id]);
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    public function get_watch_histories($user_id = 0, $course_id = 0)
    {
        if ($user_id > 0) {
            $this->db->where('student_id', $user_id);
        }
    ...
    public function update_last_played_lesson($course_id = "", $lesson_id = "")
    {
        $user_id = $this->session->userdata('user_id');
        $query   = $this->db->get_where('watch_histories', ['course_id' => $course_id, 'student_id' => $user_id]);
        ...
        if ($course_id != "") {
            if ($query->num_rows() > 0) {
                $this->db->where('watch_history_id', $query->row('watch_history_id'));
                $this->db->update('watch_histories', ['watching_lesson_id' => $lesson_id, 'date_updated' => time()]);
            } else {
                if ($lesson_id == "") {
                    $this->db->where('course_id', $course_id);
                    $this->db->order_by('order', 'asc');
    ...
    public function update_watch_history_with_duration()
    {
        $user_id                    = $this->session->userdata('user_id');
        $course_progress            = 0;
        $is_completed               = 0;
        ...
        if ($current_history->num_rows() > 0) {
            ...
            if (! is_array($watched_duration_arr)) {
                $watched_duration_arr = [];
            }
            ...
            $this->db->update('watched_duration', ['watched_counter' => $watched_duration_json, 'current_duration' => $current_duration]);
        } else {
            $watched_duration_arr     = [$current_duration];
        ...
        $lesson_total_duration = $this->db->get_where('lesson', ['id' => $data['watched_lesson_id']])->row('duration');
        ...
        if ($is_completed == 1) {
            $query           = $this->db->get_where('watch_histories', ['course_id' => $data['watched_course_id'], 'student_id' => $data['watched_student_id']]);
            $course_progress = $query->row('course_progress');
            ...
            if ($query->num_rows() > 0) {
                ...
                if (! in_array($data['watched_lesson_id'], $lesson_ids)) {
                    ...
                    $total_lesson    = $this->db->get_where('lesson', ['course_id' => $data['watched_course_id']])->num_rows();
                    $course_progress = (100 / $total_lesson) * count($lesson_ids);
        ...
        return json_encode(['lesson_id' => $data['watched_lesson_id'], 'course_progress' => round($course_progress), 'is_completed' => $is_completed]);
    }
    ...
    public function forgot_password()
    {
        ...
        $this->db->update('users', ['verification_code' => $verification_code, 'last_modified' => time()]);
    ...
    public function change_password_from_forgot_passord($verification_code = "")
    {
        ...
        $this->db->update('users', ['password' => sha1($this->input->post('new_password')), 'verification_code' => $new_verification_code]);
        return true;
    }
    ...
    public function get_categories_with_blog_number($limit = 10)
    {
        $query = $this->db
    ...
    public function update_blog_status($blog_id = "")
    {
        ...
        if ($current_status == 1):
            $this->db->where('blog_id', $blog_id);
            $this->db->update('blogs', ['status' => 0]);
        else:
            $this->db->where('blog_id', $blog_id);
            $this->db->update('blogs', ['status' => 1]);
        endif;
    }
    ...
    public function save_drip_content_settings()
    {
    ...
    public function get_custom_pages($custom_page_id = '', $button_position = "")
    {
        if ($custom_page_id > 0) {
    ...
    public function update_custom_page($custom_page_id = "")
    {
        $data['page_title']      = htmlspecialchars_($this->input->post('page_title'));
        $data['page_content']    = htmlspecialchars_($this->input->post('page_content', false));
        $data['page_url']        = slugify($this->input->post('page_url'));
    ...
    public function delete_custom_page($custom_page_id = "")
    {
        $this->db->where('custom_page_id', $custom_page_id);
        $this->db->delete('custom_page');
    }

    public function is_course_instructor($course_id = "", $user_id = "0")
    {
        $course_details = $this->get_course_by_id($course_id)->row_array();
    ...
    //Notification start
    public function get_notifications($id = "", $status = null)
    {
        if ($id > 0) {
            $this->db->where('id', $id);
        }
    ...
    public function my_notifications($user_id = "", $status = null)
    {
        if ($user_id == '') {
            $user_id = $this->session->userdata('user_id');
        }
    ...
    public function notification_enable_diable()
    {
        $id                = $this->input->post('id');
    ...
    public function update_website_faq()
    {
        $faqs = [];
    ...
    public function update_motivational_speech()
    {
        $motivations = [];
        $images      = [];
    ...
    public function get_related_courses($parent_category = "", $child_category = "", $current_course_id = "", $limit = "10")
    {
        $scorm_status = addon_status('scorm_course');
        $h5p_status   = addon_status('h5p');
    ...
    public function get_pepople_ratings($limit = 10)
    {
        $this->db->where('rating', 5);
    ...
    public function course_page_accessibility($course_id = "", $user_id = "")
    {
        if ($course_id == "") {
            return false;
        }
    ...
    public function get_course_instructors_id($course_id = "")
    {
        $row = $this->get_course_by_id($course_id);
        if ($row->num_rows() > 0) {
    ...
    public function update_custom_codes()
    {
    ...
    public function update_home_page_settings($key = "")
    {
    ...
    public function assignEmailToSendList($emails = [], $subject = "", $description = "")
    {
    ...
    // BBB Function to make API calls
    public function callBbbApi($endpoint, $data, $meeting_id)
    {
    ...
    public function checkBbbMeetingExists($endpoint, $meeting_id, $query_data = [])
    {
        $secret = get_settings('bbb_setting', true)['secret'] ?? '';
        //Sanitize API URL START
        $api_url = get_settings('bbb_setting', true)['endpoint'] ?? '';
    ...
    public function join_bbb_meeting_by_curl_calls($course_id = "", $is_moderator = false)
    {
    ...
    public function get_seo_meta_tags()
    {
        return $this->db->get('seo_fields');
    }

    public function save_seo_settings($route = "")
    {
    ...
    public function get_posts($user_id = null, $search = '')
    {
        $this->db->select('community_posts.*, users.first_name, users.last_name');
        $this->db->from('community_posts');
        $this->db->join('users', 'users.id = community_posts.user_id');

        if ($user_id) {
            $this->db->where('community_posts.user_id', $user_id);
        }
        ...
        if (! empty($search)) {
            ...
            $this->db->like('community_posts.description', $search);
        ...
        $this->db->order_by('community_posts.id', 'DESC');

        $posts = $this->db->get()->result_array();

        foreach ($posts as &$post) {

            $post['total_likes'] = $this->db
                ->where('post_id', $post['id'])
                ->count_all_results('community_post_likes');

            $post['total_comments'] = $this->db
                ->where('post_id', $post['id'])
                ->count_all_results('community_post_comments');

            $comments = $this->db
                ->select('community_post_comments.*, users.first_name, users.last_name')
                ->from('community_post_comments')
                ->join('users', 'users.id = community_post_comments.user_id')
                ->where('community_post_comments.post_id', $post['id'])
                ->where('community_post_comments.parent_id', 0)
                ->order_by('community_post_comments.id', 'DESC')
                ->get()
                ->result_array();

            foreach ($comments as &$comment) {

                $comment['replies'] = $this->db
                    ->select('community_post_comments.*, users.first_name, users.last_name')
                    ->from('community_post_comments')
                    ->join('users', 'users.id = community_post_comments.user_id')
                    ->where('community_post_comments.parent_id', $comment['id'])
                    ->order_by('community_post_comments.id', 'ASC')
                    ->get()
                    ->result_array();
            }
    ...
    {
        if (
            empty(trim($this->input->post('description'))) &&
        ...
        if (! empty($_FILES['file']['name'])) {
            ...
            $filename = md5(rand(10000000, 99999999)) . '.' . $extension;
            ...
            $data['file'] = 'uploads/posts/' . $filename;
        }

        return $this->db->insert('community_posts', $data);
    }

    public function delete_post($id)
    {
        $post = $this->db
            ->get_where('community_posts', ['id' => $id])
            ->row_array();
        ...
        $this->db->delete('community_post_likes');

        // Delete bookmarks
        $this->db->where('post_id', $id);
        $this->db->delete('community_saved_posts');

        // Delete comments
        $this->db->where('post_id', $id);
        $this->db->delete('community_post_comments');

        // Delete uploaded file
        if (! empty($post['file'])) {

            $file_path = FCPATH . $post['file'];
        ...
        // Delete post
        $this->db->where('id', $id);
        $this->db->delete('community_posts');
    ...
    public function toggle_like($post_id, $user_id)
    {
        ...
        if ($like) {
            ...
            $this->db->insert('community_post_likes', [
                'post_id' => $post_id,
                'user_id' => $user_id,
            ]);
    ...
    public function add_comment($post_id, $user_id)
    {
        ...
        $this->db->insert('community_post_comments', $data);
    ...
    public function toggle_save_post($post_id, $user_id)
    {
        $post = $this->db
            ->get_where('community_posts', [
                'id' => $post_id,
        ...
        $saved_post = $this->db
            ->get_where('community_saved_posts', [
                'post_id' => $post_id,
                'user_id' => $user_id,
            ])
            ->row_array();
        ...
        if ($saved_post) {
            ...
            $this->db->where('id', $saved_post['id']);
            $this->db->delete('community_saved_posts');
        ...
        $this->db->insert('community_saved_posts', [
            'post_id' => $post_id,
            'user_id' => $user_id,
        ]);
    ...
    public function get_saved_posts($user_id)
    {
        $this->db->select('community_posts.*, users.first_name, users.last_name');
        $this->db->from('community_saved_posts');
        $this->db->join('community_posts', 'community_posts.id = community_saved_posts.post_id');
        $this->db->join('users', 'users.id = community_posts.user_id');
        $this->db->where('community_saved_posts.user_id', $user_id);
        $this->db->order_by('community_saved_posts.id', 'DESC');
    ...
    {
        $announcements = $this->db->order_by('id', 'DESC')->get('announcements')->result_array();
        ...
        foreach ($announcements as &$announcement) {
            ...
            $courses = $this->db
                ...
                ->join('course', 'course.id = announcement_courses.course_id')
                ->where('announcement_courses.announcement_id', $announcement['id'])
                ->get()
                ->result_array();

            $course_names = array_column($courses, 'title');

            $announcement['course_names'] = implode(', ', $course_names);
        }
    ...
    public function add_announcement()
    {
        ...
        $data['is_pinned']   = $this->input->post('is_pinned') ? 1 : 0;
        $data['status']      = 1;
        ...
        if (! empty($course_ids)) {
            ...
            foreach ($course_ids as $course_id) {
                ...
                $this->db->insert('announcement_courses', [
    ...
    {
        $announcement = $this->db->get_where('announcements', [
            'id' => $announcement_id,
        ])->row_array();
        ...
        if (! empty($_FILES['attachment']['name'])) {
            ...
            if ($this->upload->do_upload('attachment')) {
                ...
                if (! empty($announcement['attachment'])) {
                    ...
                    $old_file = $upload_path . $announcement['attachment'];
        ...
        $this->db->delete('announcement_courses');
        ...
        $this->session->set_flashdata(
            'flash_message',
            get_phrase('updated_successfully')
        );
    }
    ...
    public function delete_announcement($announcement_id = "")
    {
        ...
        if (! empty($announcement['attachment'])) {
            ...
            $file_path = FCPATH . 'uploads/announcements/' . $announcement['attachment'];
        ...
        // Delete related course mappings
        $this->db->where('announcement_id', $announcement_id);
        $this->db->delete('announcement_courses');
