        ...
        // ✅ Extract only real HTML (skip <html>
            ...
            // ✅ Extract only real HTML (skip <html>
                ...
                // ✅ Save cleaned section (no <html>
                    ...
                    public function announcements($param1 = "", $param2 = "")
                    {
                        if ($this->session->userdata('admin_login') != true) {
                            redirect(site_url('login'), 'refresh');
                        }
                        ...
                        // CHECK ACCESS PERMISSION
                        check_permission('announcements');                    

                        if ($param1 == "add") {
                            $this->crud_model->add_announcement();
                            redirect(site_url('admin/announcements'), 'refresh');
                        } elseif ($param1 == "edit") {
                            $this->crud_model->edit_announcement($param2);
                            redirect(site_url('admin/announcements'), 'refresh');
                        } elseif ($param1 == "delete") {
                            $this->crud_model->delete_announcement($param2);
                            redirect(site_url('admin/announcements'), 'refresh');
                        }

                        $page_data['page_name']  = 'announcements';
                        $page_data['page_title'] = get_phrase('announcements');
                        $page_data['announcements'] = $this->crud_model->get_announcements();
                        $this->load->view('backend/index', $page_data);
                    }

                    public function announcement_form($param1 = "", $param2 = "")
                    {
                        if ($this->session->userdata('admin_login') != true) {
                            redirect(site_url('login'), 'refresh');
                        }
                        ...
                        if ($param1 == 'add_announcement_form') {
                            $page_data['page_name']  = 'announcement_add';
                            $page_data['page_title'] = get_phrase('announcement_add');
                            $this->load->view('backend/index', $page_data);
                        } elseif ($param1 == 'edit_announcement_form') {
                            $page_data['page_name']  = 'announcement_edit';
                            $page_data['announcement_id']    = $param2;
                            $page_data['page_title'] = get_phrase('announcement_edit');
                            $this->load->view('backend/index', $page_data);
                        }
                    }
